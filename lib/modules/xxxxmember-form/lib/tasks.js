...
module.exports = function(self,options){
    ...
    self.runMigrations = function (apos ,data , id, callback) {
        return apos.migrations.eachDoc({
            ...
            var contactFilter = _.pick(contact, ["personName", "email", "message"]);
            var validate = _.reduce(dataFilter, function (result, value, key) {
                return _.isEqual(value, contactFilter[key]) ?
                    true : false;
            }, {});
            // Run if matches
            if(validate){
                console.log("\n\nSuccessfully Removed : \n", JSON.stringify(contactFilter, undefined, 2));
                return apos.docs.db.remove(contactFilter ,callback);
            }else{
                // If not , pass by only
