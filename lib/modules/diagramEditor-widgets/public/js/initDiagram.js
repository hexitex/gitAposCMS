apos.define('diagramEditor-widgets', {
    ...
    construct: function (self, options) {
        ...
        self.play = function ($widget, data, options) {
           var dtaa = self.getData($widget)
           var diagram=null;
           // generate unq name
           if (dtaa.diagramName=='621389123768')
           {
               dtaa.diagramName=new Date().getTime()+Math.round(Math.random(100000)*(Math.random(100000)*100000));
               $widget.find('#621389123768').attr("id", dtaa.diagramName);
        
            }
           if (idsFound.indexOf(dtaa.diagramName)>-1)
           {
               //need to rename this now - prob a clone operation
               var newdiagramName=new Date().getTime()+Math.round(Math.random(100000)*(Math.random(100000)*100000));
               $widget.find('#'+dtaa.diagramName).attr("id", newdiagramName);
               idsFound.push(newdiagramName);
               dtaa.diagramName=newdiagramName;
            
           }
           else
           {
              idsFound.push(dtaa.diagramName);
           }
           diagram = $widget.find('#'+dtaa.diagramName);
            diagram.click(function() { DiagramEditor.editElement(diagram[0]); })



            DiagramEditor.editElement = function (diagram, config, ui, done) {
        
                if (!diagram.diagramEditorStarting) {
                    diagram.diagramEditorStarting = true;

                    return new DiagramEditor(config, ui, done, function () {
                        delete diagram.diagramEditorStarting;
                    }).editElement(diagram);
                }
            };
             ...
             */
            DiagramEditor.prototype.frameStyle = 'z-index:999;position:fixed;top:0;left:0;border:0;width:100%;height:100%;'
            ...
            DiagramEditor.prototype.setElementData = function (elem, data) {
                ...
                //save back to widget
                dtaa.svgSource = elem.attributes.src.value;
                
                self.setData($widget, dtaa);
                try{self.startAutosavingAreaThen($widget)}catch(ex){}
                return elem;
            };
             ...
             * Invoked after save.
             */
            DiagramEditor.prototype.done = function (data, draft, elm) {
                // hook for subclassers
                // dta.svgSource=data;
                //$widget.find(diagram).src=DiagramEditor.prototype.getData();
