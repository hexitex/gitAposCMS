...
module.exports = {
  ...
  construct: function (self, options) {
    if (!options.clientID || !options.secret || !options.mode) {
      console.error('WARNING: missing clientID, secret or mode in ecom-paypal module config');
    }
    // Paypal variables
    ...
    self.apos.app.post('/paypal/complete', async function (req, res, next) {
      try {
        ...
        await self.apos.docs.getManager('ecom-orders').addOrder(req, info);

        const sendEmailOrder = Promise.promisify(self.sendEmailOrder);
        await sendEmailOrder(req, info, 'orderCompleted', options.emailDefaultFrom, req.user.email, 'You order has been received');
